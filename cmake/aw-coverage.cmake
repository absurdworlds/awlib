...
if (CMAKE_CXX_COMPILER_ID MATCHES "Clang")
	...
	# Every process writes its own raw profile; %p keeps two tests running
	# under `ctest -j` from writing over each other. The path is compiled in
	# rather than handed to each test through LLVM_PROFILE_FILE, so that it
	# holds for every instrumented process -- a test registered with a plain
	# add_test(), a fuzzer, a helper a test spawns -- and not only for the
	# ones that went through aw_add_test(). The variable still overrides it.
	set(profile_file "${coverage_dir}/%p.profraw")
	add_compile_options(-fprofile-instr-generate=${profile_file} -fcoverage-mapping -O0 -g)
	add_link_options(-fprofile-instr-generate=${profile_file})
