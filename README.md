...
they point at are in `cmake/sanitizers/`, and are for leaks and races
...
how CI runs GCC's analyser and cppcheck: both are clean, and this keeps them
that way. clang-tidy still has findings to work through, so CI reports those
instead -- as a summary on the run, and as SARIF for GitHub's code scanning.
`compile_commands.json` is written by default, so the analysers can equally
be run out of band -- `run-clang-tidy -p build`.
